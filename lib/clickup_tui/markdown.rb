    ...
    class << self
      ...
      def simple_format(content, width)
        ...
        lines.each do |line|
          ...
          when /^# (.+)$/
            formatted_lines << ''
            formatted_lines << "\e[1m\e[4m#{::Regexp.last_match(1)}\e[0m"
            formatted_lines << ''
          when /^## (.+)$/
            formatted_lines << ''
            formatted_lines << "\e[1m#{::Regexp.last_match(1)}\e[0m"
            formatted_lines << ''
          when /^### (.+)$/
            formatted_lines << ''
            formatted_lines << "\e[1m#{::Regexp.last_match(1)}\e[0m"
          when /^\* (.+)$/, /^- (.+)$/
            formatted_lines << "  • #{::Regexp.last_match(1)}"
          when /^\d+\. (.+)$/
            formatted_lines << "  #{::Regexp.last_match(0)}"
          when /^```/
            # Simple code block handling
            formatted_lines << if line == '```'
                               end
            "\e[2m#{line}\e[0m"
          when /^`([^`]+)`/
            # Inline code
